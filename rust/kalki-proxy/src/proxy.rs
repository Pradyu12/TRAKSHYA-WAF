...
pub async fn handle_proxy_request(
    ...
    if let Err(block) = inspector.inspect(
        &uri_path, &query, &method, &body_bytes, &client_ip
    ).await {
        return block;
    }
    ...
    forward_to_upstream(&upstream_url, &method, &uri_path, &query, parts.headers, body_bytes).await
}
