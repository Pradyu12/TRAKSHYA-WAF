...
impl RequestInspector {
    ...
    pub async fn inspect(&self, uri_path: &str, query: &str, method: &str, body: &[u8], client_ip: &str) -> Result<(), Response> {
        ...
        let rules_engine = krsna_rules::Engine::new();
        if let Some(rule) = rules_engine.check_attack(&combined) {
            ...
            tokio::spawn(async move {
                if let Err(e) = crate::db::record_incident(&state_clone.db_pool, &incident_clone).await {
                    tracing::error!("Failed to record incident: {}", e);
                }
                if let Err(e) = crate::db::record_request(&state_clone.db_pool, &client_ip_owned, true).await {
        ...
        tokio::spawn(async move {
            if let Err(e) = crate::db::record_request(&state_clone.db_pool, &client_ip_owned, false).await {
                tracing::error!("Failed to record request: {}", e);
            }
        });

        if cfg.proxy.posture != Posture::Monitor && cfg.rate_limiter.enabled {
            let limiter = krsna_rate_limiter::RateLimiter::new(
