...
use crate::db::{DashboardStats, GeoData, SIEMAStats, Rule, VulnStats, Incident, BlacklistEntry, AttackCount, GeoLocation, SeverityCounts};
use chrono::Utc;
use uuid::Uuid;
...
async fn update_config(
    ...
    // Persist to database
    if let Err(e) = crate::db::update_config(&state.db_pool, "posture", &format!("{:?}", cfg.proxy.posture)).await {
...
async fn set_posture(
    ...
    if let Err(e) = crate::db::update_config(&state.db_pool, "posture", &format!("{:?}", posture)).await {
        tracing::error!("Failed to persist posture: {}", e);
    }
...
async fn get_dashboard_stats(
    ...
    let rule_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM rules")
        ...
        .await
        .unwrap_or(0);
    let stats = crate::db::get_dashboard_stats(&state.db_pool, &format!("{:?}", cfg.proxy.posture), rule_count, state.uptime_seconds() as i64).await
...
async fn get_geo(
    ...
    check_auth(&headers, &state).await?;
    let data = crate::db::get_geo_data(&state.db_pool).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    Ok(Json(data))
}
...
async fn get_siem_stats(
    ...
    check_auth(&headers, &state).await?;
    let stats = crate::db::get_siem_stats(&state.db_pool).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    Ok(Json(stats))
}
...
async fn get_siem_alerts(
    ...
    let alerts = crate::db::get_siem_alerts(&state.db_pool, limit, offset).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    Ok(Json(alerts.into_iter().map(|a| serde_json::json!({
        "id": a.id,
...
async fn get_rules(
    ...
    check_auth(&headers, &state).await?;
    let rules = crate::db::get_rules(&state.db_pool).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    Ok(Json(rules))
}
...
async fn create_rule(
    ...
    if regex::Regex::new(&req.pattern).is_err() {
        return Err((StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "Invalid regex pattern"}))));
    }
    ...
    crate::db::create_rule(&state.db_pool, &rule).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    
    Ok(Json(serde_json::json!({"status": "ok", "message": "Rule created"})))
}
...
async fn delete_rule(
    ...
    check_auth(&headers, &state).await?;
    crate::db::delete_rule(&state.db_pool, &id).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    Ok(Json(serde_json::json!({"status": "ok", "message": "Rule deleted"})))
}
...
async fn test_rule(
    ...
    let re = match regex::Regex::new(&req.pattern) {
        Ok(r) => r,
        Err(e) => return Ok(Json(serde_json::json!({"error": format!("Invalid regex: {}", e)}))),
    };
    
    let matches = re.find_iter(&req.payload)
...
async fn get_blacklist(
    ...
    check_auth(&headers, &state).await?;
    let list = crate::db::get_blacklist(&state.db_pool).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    Ok(Json(list.into_iter().map(|b| serde_json::json!({
        "id": b.id,
        "ip": b.ip,
        "reason": b.reason,
...
async fn add_blacklist(
    ...
    check_auth(&headers, &state).await?;
    crate::db::add_to_blacklist(&state.db_pool, &req.ip, req.reason.as_deref()).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    Ok(Json(serde_json::json!({"status": "ok", "message": "IP added to blacklist"})))
}
...
async fn remove_blacklist(
    ...
    check_auth(&headers, &state).await?;
    crate::db::remove_from_blacklist(&state.db_pool, &ip).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    Ok(Json(serde_json::json!({"status": "ok", "message": "IP removed from blacklist"})))
}
...
async fn get_vuln_stats(
    ...
    check_auth(&headers, &state).await?;
    let stats = crate::db::get_vuln_stats(&state.db_pool).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    Ok(Json(stats))
}
...
async fn get_vulns(
    ...
    check_auth(&headers, &state).await?;
    let vulns = crate::db::get_vulnerabilities(&state.db_pool).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    Ok(Json(vulns))
}
...
async fn run_vuln_scan(
    ...
    Ok(Json(serde_json::json!({"status": "started", "scan_id": "scan-123"})))
}
...
async fn sse_handler(
    ...
    let stream = BroadcastStream::new(rx).map(|msg| {
        msg.map(|data| Event::default().json_data(data).unwrap())
    });
...
async fn simulate_attack(
    ...
    crate::db::record_incident(&state.db_pool, &incident).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": e.to_string()}))))?;
    ...
    Ok(Json(serde_json::json!({"status": "ok", "message": "Attack simulated"})))
}
