...
impl GeoIp {
    pub fn new(db_path: Option<&Path>) -> Self {
        let reader = db_path.and_then(|p| Reader::open_readfile(p).ok());
    ...
    pub fn lookup(&self, ip: &str) -> Option<GeoIpResult> {
        ...
        let city = result
            ...
            .and_then(|c| c.names.as_ref())
            .and_then(|n| n.get("en").map(|s| s.to_string()));
        let latitude = result.location.as_ref().and_then(|l| l.latitude);
        let longitude = result.location.as_ref().and_then(|l| l.longitude);
        let is_proxy = result
            .traits
